...
describe Decidim::SecureElections::ElectionStatusCell do
  ...
  let(:election_class) { Struct.new(:status, :start_time, :end_time, keyword_init: true) }

  def state_for(status:, start_time: nil, end_time: nil)
    described_class.state_for(election_class.new(status:, start_time:, end_time:))
  end
  ...
  describe ".state_for" do
    ...
    it "reports a ready election inside its calendar as ongoing" do
      expect(state_for(status: "ready", start_time: 1.hour.ago, end_time: 1.hour.from_now)).to eq(:ongoing)
    end

    it "reports a ready election before its start time as scheduled" do
      expect(state_for(status: "ready", start_time: 1.hour.from_now, end_time: 2.hours.from_now)).to eq(:scheduled)
    end
    ...
    it "reports a ready election past its end time as ended" do
      expect(state_for(status: "ready", start_time: 2.hours.ago, end_time: 1.hour.ago)).to eq(:ended)
    end

    it "reports a paused election as paused even inside its calendar" do
      expect(state_for(status: "paused", start_time: 1.hour.ago, end_time: 1.hour.from_now)).to eq(:paused)
    end
    ...
    it "reports a published tally as results" do
      expect(state_for(status: "results", start_time: 2.hours.ago, end_time: 1.hour.ago)).to eq(:results)
    end

    it "reports a canceled election as canceled even inside its calendar" do
      expect(state_for(status: "canceled", start_time: 1.hour.ago, end_time: 1.hour.from_now)).to eq(:canceled)
    end
